...
use crate::claude::{self, ClaudeClient};
...
impl Decider {
    pub fn new(selection: &Selection) -> Result<Self> {
        ...
        Ok(match selection.kind {
            DeciderKind::Jev => Self::Jev(JevClient::new(openrouter_key()?, model)),
            DeciderKind::Claude => Self::Claude(ClaudeClient::new(model)),
        })
    }
    ...
    pub fn decide(
        ...
        match self {
            ...
            Self::Claude(client) => client.decide(state, questions),
        }
    }
    ...
    pub fn model(&self) -> &str {
        match self {
            ...
            Self::Claude(client) => client.model(),
        }
    }
}
...
fn openrouter_key() -> Result<String> {
    std::env::var("OPENROUTER_API_KEY")
        .map_err(|_| anyhow!("OPENROUTER_API_KEY is not set; export an OpenRouter API key to ask Jev"))
}
